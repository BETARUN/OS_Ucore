...
1. 物理内存的探测。这是本实验的基础，从探测结果可以看到内存空间分为保留空间和可用空间，而可用空间由分为内核保留空间和空闲空间，因此装机的内存实际上并不是所有空间都可以让应用程序使用，在OS原理中并没有提及这一点
2. 对物理内存的分页管理。这是本实验的核心，对应到OS原理中也是物理内存管理中的一个重点，通过分页管理可以避免内存外碎片，有助于增加内存利用率，在OS原理中还有分段管理的不少内容，在本实验中并没有直接涉及，大概这个本身内容也不多，在实验中内核的末尾区域存放了页描述符，这在OS原理中也没有提及，造成逻辑上的页与实际的内存空间缺少中间的联系
...
这次实验需要实现对物理内存的页式管理，对比上一个实验会多出很多需要理解的概念和数据结构，不过文档的说明也比较详细，参考文档以及memlayout.h, mmu.h和pmm.h文件能够对这个本次实验所需要的数据结构有所了解

接着是追踪物理内存管理的初始化函数的调用过程了解系统的行为以及对数据结构的使用，这里的难点是练习1，主要的难点是页释放函数的实现，这需要对相应的数据结构和行为都有足够了解，好在本实验提供了详细的检查函数用于检测实现是否正确，可以方便地定位到出错位置，再结合一些流程分析就可以比较方便地排查错误，当然了这也要求我进一步熟悉调试工具的使用和流程分析

还有一个被坑到的就是练习1中的页分配函数是不完整的，跟页释放函数的很明显不完整不一样，页分配函数只缺一点点，这就造成容易在后面实现页释放函数时忘记了这件事导致出现问题

总的来说，这次实验让我对操作系统的理解更上一个台阶，特别是其中页描述符的部分在OS原理中没有提及，是相当重要的补充

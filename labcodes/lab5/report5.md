...
要注意的是本实验要去除时钟中断处理函数中的屏幕打印函数，否则结果会出错
...
这个实验要求完成操作系统对用户进程的管理以及系统调用的实现，首先要说的是系统调用接口的实现，在实验1对中断的设置上留了一个用户态陷入内核态的系统调用接口，操作系统解析调用参数将功能分发到各个功能模块，完成系统调用的实现

在系统内核外部，本实验对系统调用接口进行封装形成应用程序库，这样应用程序就可以方便地通过库函数请求系统调用

在这个实验中值得注意的还有对中断帧的处理，只有正确设置中断帧才能让系统调用返回时能回到正确的执行位置，或是让程序开始执行

对于用户进程管理部分，主要处理的是各进程之间的相互关系，包括父进程对子进程的等待和资源回收等，在内核接管CPU时需要确保完成相应的任务
